...
public class Zone implements java.io.Serializable, Iterable<Card> {
    ...
    protected final transient Multimap<ZoneType, Card> cardsAddedThisTurn = MultimapBuilder.enumKeys(ZoneType.class).arrayListValues().build();
    protected final transient Multimap<ZoneType, Card> cardsAddedLastTurn = MultimapBuilder.enumKeys(ZoneType.class).arrayListValues().build();
    ...
    private static List<Card> getCardsAdded(final Multimap<ZoneType, Card> cardsAdded, final ZoneType origin) {
        if (origin != null) {
